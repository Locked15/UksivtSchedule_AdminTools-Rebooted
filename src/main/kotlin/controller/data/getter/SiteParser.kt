...
class SiteParser {
     ...
     * and seeks all available [targetChangesOfDay nodes][ChangeElement].
     *
    ...
    /**
     * Executes page queries (like CSS-Selector query) and extracts targetChangesOfDay container children.
    ...
    private fun calculateNewMonthBeginning(element: Element, model: BaseIteratorModel) {
        ...
        if (element.nodeName() == "p" && checkValueToMonth(element.text())) {
            ...
            model.currentMonth = element.text().substring(0, element.text().lastIndexOf(' '))
    ...
    private fun checkFirstCellAndUpdateObjects(cell: Element, iterator: InnerIteratorModel): Boolean {
        var result = false
        if (getBasicSiblingIndex(cell.siblingIndex()) == 0) {
    ...
    companion object {
        ...
        /**
         * Path to official college website, targeted to targetChangesOfDay page.
         */
        ...
        /**
         * Calculates [basic][Int] [sibling index][Element.siblingIndex] for the [element][Element].
         *
         * Cause final index calculates by formula: "{index} * 2 + 1";
         * So I reversed it and the calculus basic index by follow: "({final_index} - 1) / 2".
         */
        private fun getBasicSiblingIndex(index: Int) = (index - 1) / 2

        /**
         * Checks [current value][value] to be one of the months.
         * [Returns][Boolean] 'true' if yes, and 'false' otherwise.
         */
        private fun checkValueToMonth(value: String) = when (value.lowercase()) {
